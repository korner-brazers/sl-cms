...
class mail
{
	...
	*/
	...
	function From( $from )
	{
		...
		if( ! is_string($from) ) {
			$this->sl->fn->info("ошибка, From должен быть строкой");
		}
	...
	function CheckAdresses( $aad )
	{
		for($i=0;$i< count( $aad); $i++ ) {
			if( ! $this->ValidEmail( $aad[$i]) ) {
				$this->sl->fn->info("ошибка : не верный email ".$aad[$i]);
			}
		}
	}
	...
	function _build_attachement()
	{
		...
		for( $i=0; $i < count( $this->aattach); $i++ ) {
			...
			if( ! file_exists( $filename) ) {
				return "ошибка прикрепления файла : файл $filename не существует";
			}
