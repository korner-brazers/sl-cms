...
function <?=$_GET['module']?>_next(p,id,ver){
    if(p == 4) ver = $('input[name="bridge_ver"]').val();
    
    $.sl('shell',{name:'<?=$_GET['module']?>',add_param:p+'/'+(id || 0)+'/'+(ver || ''),method:'step',post:{login:bridge_l,pass:bridge_p}},'update');
}

$('ul.bridge_apps_list li').live('click',function(){
    <?=$_GET['module']?>_next(2,$(this).attr('id'),$(this).attr('ver'));
})
